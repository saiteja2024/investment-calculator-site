      ...
      <div className="wrap">
        ...
        <div className="layout">
          <section className="controls">
            <NumberField label="Starting amount" value={initial} onChange={setInitial} />
            <NumberField label="Monthly contribution" value={monthly} onChange={setMonthly} />
            <SliderField label="Time horizon" value={years} onChange={setYears} min={1} max={40} step={1} display={`${years} yrs`} />
            <SliderField label="Expected annual return" value={rate} onChange={setRate} min={1} max={15} step={0.5} display={`${rate.toFixed(1)}%`} />
            <small>10% approximates the S&amp;P 500&apos;s long-run historical average. Actual returns vary year to year.</small>
            <div className="divider">
              <button className="toggle-row" onClick={() => setAdjustInflation(!adjustInflation)}>
            ...
            <div className="charts">
              <div>
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <AreaChart data={result.series} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                    <CartesianGrid stroke={COLORS.hair} vertical={false} />
                    <XAxis dataKey="year" tickFormatter={(value) => `${value}y`} stroke={COLORS.paperDim} />
                    <YAxis tickFormatter={(value) => `$${Math.round(value / 1000)}k`} stroke={COLORS.paperDim} />
